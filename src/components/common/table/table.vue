<template>
    <div>
        ...
        <el-form
                ...
                ref="mainDialog"
        >
            <!--:data="[{isFitS: true}].concat(data).concat([{isFitE: true}])"-->
            <el-table
                            ...
                            <template v-for="(it1,is) in it.children">
                                <el-table-column
                                    ...
                                    <template slot-scope="scope">
                                        <div class="cell-editor" v-if="typeof it1.editor === 'function' ? it1.editor(scope.row) : it1.editor"
                                            ...
                                            <template
                                                ...
                                                <el-tooltip class="item" effect="dark" :content="$t('lang.common.save')"
                                                            placement="top" :key="i">
                                                    <el-button type="success" size="mini" @click="save(scope.row)"
                                                               circle>
                                                ...
                                                <el-tooltip v-if="!editorAddItem" class="item" effect="dark"
                                                    ...
                                                    <el-button type="primary" size="mini" @click="reset(scope.row)"
                                                               circle>
                                        ...
                                        <template
                                                v-else-if="((!isEditor || editorRowId !== scope.row.id || !dialog.option.find(its => (its.tableField === it1.prop || its.field === it1.prop))) && !it1.editor) && (scope.row.id !== editorAddItem)  && (it1.prop === 'lastUpdateTime' || it1.prop === 'createTime' || it1.prop === 'effectiveTime' || it1.prop === 'expiryTime' || it1.prop ==='updateTime' || it1.prop === 'validEndTime' || it1.prop === 'validFromTime' || it1.prop === 'ciDate' || it1.prop === 'verifyDate' || it1.prop === 'tradeTime' || it1.prop === 'pklDate' || it1.prop === 'loadDate'|| it1.prop === 'etd' || it1.prop === 'eat' || it1.prop === 'atd' || it1.prop === 'ata' || it1.prop ==='actGrDate' || it1.dateFormatter || it1.numberFormatter) ">
                                      <span class="table-cell-span">
                                      <template v-if="it1.numberFormatter">
                                        {{priceFormatter(scope.row[it1.prop], it1.numberFormatter)}}
                                      </template>
                                      <template v-else>
                                        {{it1.yyyyMMdd && dateFormatter(scope.row[it1.prop]) ? dateFormatter(scope.row[it1.prop], 'dd-MM-yyyy').substring(0, 10) : dateFormatter(scope.row[it1.prop])}}
                                      </template>
                                      </span>
                                        </template>
                                        <template
                                                v-else-if="editor &&  isEditor && (cellEditorProp === 'all' || cellEditorProp === it1.prop)  && dialog.option.findIndex(its => (its.tableField === it1.prop || its.field === it1.prop)) > -1 && editorRowId === scope.row.id">
                                            <div :style="{width: 'calc(100% - 14px)', marginLeft: '8px'}">
                                                <cx-condition
                                                        :option="dialog.option[dialog.option.findIndex(its => (its.tableField === it1.prop || its.field === it1.prop))]"
                                                        :form="dialog.formlist" type="tableEditor"></cx-condition>
                                            </div>
                                        </template>
                                        <template v-else-if="it1.defined">
                                            <slot :name="it1.prop" :row="scope.row" :value="scope.row[it1.prop]"
                                                  :index="scope.$index" :data="data"></slot>
                                        </template>
                                        <template v-else-if="it1.formatter">
                                            {{it1.formatter(scope.row[it1.prop],scope.row)}}
                                        </template>
                                        <template
                                                v-else-if="filterable && filterInput && scope.row[it1.prop].toString().indexOf(filterInput) !== -1">
                                            {{scope.row[it1.prop].toString().substring(0,scope.row[it1.prop].toString().indexOf(filterInput))}}<strong
                                                class="el-button--text">{{filterInput}}</strong>{{scope.row[it1.prop].toString().substring(scope.row[it1.prop].toString().indexOf(filterInput)+filterInput.length)}}
                                        </template>
                                        <template v-else>
                                    <span class="table-cell-span">
                                        {{scope.row[it1.prop]}}
                    ...
                    <template v-else>
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                ...
                                <template
                                        v-else-if="((!isEditor || editorRowId !== scope.row.id || !dialog.option.find(its => (its.tableField === it.prop || its.field === it.prop))) && !it.editor) && (scope.row.id !== editorAddItem)  && (it.prop === 'lastUpdateTime' || it.prop === 'createTime' || it.prop === 'effectiveTime' || it.prop === 'expiryTime' || it.prop ==='updateTime' || it.prop === 'validEndTime' || it.prop === 'validFromTime' || it.prop === 'ciDate' || it.prop === 'verifyDate' || it.prop === 'tradeTime' || it.prop === 'pklDate' || it.prop === 'loadDate'|| it.prop === 'etd' || it.prop === 'eat' || it.prop === 'atd' || it.prop === 'ata' || it.prop ==='actGrDate' || it.dateFormatter || it.numberFormatter) ">
                              <span class="table-cell-span">
                                <template v-if="it.numberFormatter">
                                    {{priceFormatter(scope.row[it.prop], it.numberFormatter, it.prop)}}
                                </template>
                                <template v-else>
                                    {{it.yyyyMMdd && dateFormatter(scope.row[it.prop]) ? dateFormatter(scope.row[it.prop], 'dd-MM-yyyy').substring(0, 10) : dateFormatter(scope.row[it.prop])}}
                                </template>
                              </span>
                                </template>
                                <template
                                ...
                                <template v-else-if="it.formatter">
                                    {{it.formatter(scope.row[it.prop],scope.row)}}
                                </template>
                                <template
                                        v-else-if="filterable && filterInput && scope.row[it.prop].toString().indexOf(filterInput) !== -1">
                                    {{scope.row[it.prop].toString().substring(0,scope.row[it.prop].toString().indexOf(filterInput))}}<strong
                                        class="el-button--text">{{filterInput}}</strong>{{scope.row[it.prop].toString().substring(scope.row[it.prop].toString().indexOf(filterInput)+filterInput.length)}}
                                </template>
                                <template v-else>
                            <span class="table-cell-span">
                                {{scope.row[it.prop]}}
